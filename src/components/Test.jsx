...
export default function CardStack() {
  ...
  const removeCard = (id) => {
    setCards((prevCards) => {
      ...
      const newCard = {
        ...
        icon: ["bed", "users", "dollar", "arrowUpRight"][Math.floor(Math.random() * 4)],
  ...
  return (
    <div className="relative mx-auto my-10 h-[400px]  w-[400px]">
...
function Card({ card, index, removeCard, getIconComponent, totalCards }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: yOffset, x: xOffset, scale: 1 - index * 0.04, rotateZ: index * -3 }}
      ...
      onDragEnd={(_, info) => {
        if (index === 0) {
          ...
          if (distance > 150) removeCard(card.id);
        }
      }}
      whileDrag={{
        scale: 1.05,
